...
// указываем что это сущность
@Entity
// указываем явно на таблицу к которой привязана эта сущность
@Table(name = "book")
public class Book {
    // указываем что это id
    @Id
    // указываем что генерироваться id будет по уникальности последовательно
    // соответствует строке в PostgreSQL запросе GENERATED BY DEFAULT AS IDENTITY
    ...
    @Column(name = "year")
    private int year;
    // указываем на связь по внешнему ключу
    @ManyToOne(fetch = FetchType.LAZY)
    // указываем что колонка для внешнего ключа называется person_id и ссылаетсяс она на id из сущности person
    @JoinColumn(name = "person_id", referencedColumnName = "id")
    private Person person;
    @Column(name = "taken_at")
    @Temporal(TemporalType.TIMESTAMP)
    private Date takenAt;
    // это поле будет игнорироваться Hibernate
    @Transient
